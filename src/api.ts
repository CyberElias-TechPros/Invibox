...
export const api={
 ...
 forgotPassword:(email:string)=>request<any>('/auth/password/forgot',{method:'POST',body:JSON.stringify({email})}),
 ...
 integrationStatus:(eventId:string)=>request<Record<string,boolean>>(`/events/${eventId}/integrations/status`),
 aiAssist:(eventId:string,task:string,prompt:string)=>request<{answer:string}>(`/events/${eventId}/ai/assist`,{method:'POST',body:JSON.stringify({task,prompt})}),
 inviteTeam:(eventId:string,email:string,role:string)=>request<{ok:boolean;delivered:boolean;demoToken?:string}>(`/events/${eventId}/team-invitations`,{method:'POST',body:JSON.stringify({email,role})}),
 acceptTeamInvite:(token:string)=>request('/team-invitations/accept',{method:'POST',body:JSON.stringify({token})}),
 ...
 initializePayment:(body:{slug:string;token?:string;email:string;amount:number;purpose:string})=>request<{checkoutUrl:string;reference:string}>('/public/payments/paystack/initialize',{method:'POST',body:JSON.stringify(body)}),
 paymentStatus:(reference:string)=>request<{status:string;amount:number;purpose:string}>(`/public/payments/${encodeURIComponent(reference)}`),
